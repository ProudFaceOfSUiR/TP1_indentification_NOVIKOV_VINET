...
Avec $\mathbf{x} = \begin{bmatrix} k_0 & k_1 & k_2 & b_0 & b_1 & b_2 & m_1 & m_2 & m_3 \end{bmatrix}^T$, les équations (1) donnent pour chaque instant $i$ :
...
Avec les 801 instants ($A$ : $2403\times9$, $\mathrm{cond}(A)\approx 3{,}2\cdot10^2$) on retrouve tous les paramètres (erreurs < 0,01 %).
...
Avec $k_1$ = 5000 N/m, $b_1$ = 400 Ns/m et tous les points, les erreurs vont de 10 à 49 % ($k_0$ reste à 0,3 %). Sans quantification on retrouve tout, c'est donc la résolution le problème : la liaison est plus raide, $\max|\alpha-\beta|$ = 37 µm (moins de 4 pas) et $\max|\dot\alpha-\dot\beta|$ = 1 mm/s (10 pas), les colonnes de $k_1$ et $b_1$ sont mal mesurées.
...
Avec $k_1 = 5\cdot10^5$ N/m et $b_1 = 4\cdot10^4$ Ns/m : $\max|\alpha-\beta|$ = 0,37 µm (27 fois moins que le pas) et $\max|\dot\alpha-\dot\beta|$ = 9,8 µm/s, ces colonnes ne sont plus que du bruit. Le modèle complet donne $k_1$ = −195 N/m (négatif !) et jusqu'à 99 % d'erreur sur les autres ($\mathrm{cond}(A)\approx 8{,}7\cdot10^4$) alors que sans quantification il retrouve $k_1$. $m_1$ et $m_2$ forment un seul bloc : $k_1$, $b_1$, et $m_1$, $m_2$ séparément, ne sont pas identifiables.
...
avec $\mathbf{x} = \begin{bmatrix} k_0 & k_2 & b_0 & b_2 & m_1+m_2 & m_3 \end{bmatrix}^T$. On enlève $t=0$ ($\ddot\alpha=1$ et $\ddot\beta=0$ : $m_1$ accélère seule pendant ≈ 12 µs, sinon 33 % d'erreur sur $m_1+m_2$) et on ne filtre pas (le filtre étale ce pic). Résultat : $\mathrm{cond}(A)$ = 58 et toutes les erreurs < 0,3 %.
